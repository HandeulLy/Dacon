...
class Simulator:
    ...
    def get_state(self, data): # 숫자가 있는 공정에 대해 해당 숫자만 추출
        if 'CHECK' in data:
    ...
    def cal_schedule_part_1(self, df):
        ...
        end_time = df_out.index[-1] # 아마 submission.csv 파일의 맨 마지막 날짜를 의미할 것.
    ...
    def cal_stock(self, df, df_order):
        ...
        for i, time in enumerate(df.index):
            ...
            if i == 0:
                df_stock.iloc[i] = df.iloc[i] # 여기랑
            else:
                df_stock.iloc[i] = df_stock.iloc[i-1] + df.iloc[i] # 여기는 무슨 소리인지 모르겠고
    ...
    def add_stock(self, df, df_stock): # sample_submission과 같은 DF 생성후 첫 행(시작날짜, 2020-04-01 00:00)에 보유량(stock.csv 값) 저장
        df_out = df.copy()
